...
class RBFN(Gaussians):
    ...
    def train_ls(self, x_data, y_data):
        ...
        G = X.T
        aux = (G.T @ y)
        self.theta = np.linalg.inv(G.T @ G) @ G.T @ y
    ...
    def train_ls2(self, x_data, y_data):
        ...
        for i in range(len(x_data)):
            a = a + (self.phi_output(x_data[i]) @ self.phi_output(x_data[i]).T)
            b = b + (self.phi_output(x_data[i]).T[0] * y_data[i])
    ...
    def train_gd(self, x, y, alpha):
        ...
        X = self.phi_output(x)
        f = X.T[0] @ self.theta
        self.theta = self.theta + alpha * (y - f) * X.T[0]
    ...
    def residuals(self, x_data, y_data):
        ...
        d = []
        for i in range(len(x_data)):
            d.append(self.f(x_data[i]) - y_data[i])
        return np.sum(np.array(d)**2)
